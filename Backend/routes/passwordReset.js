...
router.get("/password-reset/:userId/:token", async (req, res) => {
  try {
    ...
    const newPass = CryptoJS.AES
      .encrypt(formattedNumber.toString(), process.env.PASS_SEC)
      .toString();
    user.password = newPass;
    await user.save();
    await Token.deleteOne({userId: token.userId});

    res.send(`Password reset successfully <br>New password: ${formattedNumber} <br>Please change your password when entering the application`);
  } catch (error) {
    res.send("An error occured");
    console.log(error);
  }
