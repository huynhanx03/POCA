...
router.post("/:channelId/add-subscribe/:userId", async (req, res) => {
  ...
  try {
    ...
    console.log(updatedChannel.value.idUser);
    if(updatedChannel.value.isUser) {
      const hostUser = await User.findById(updatedChannel.value.idUser);
      if(hostUser && hostUser.fcmToken) {
        await sendNotify("New User Subscribed", `${user.fullName} has already subscribed your channel. Check it out!`, "subscribe", hostUser.fcmToken);
      }
    }
