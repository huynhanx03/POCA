...
router.post("/", verifyTokenAndAuthorization, async (req, res) => {
  ...
  try {
    ...
    const channel = await Channel.findById(host);
    const subscribedUsers = await User.find({ _id: { $in: channel.subscribed } });
   const uniqueFcmTokens = new Set();
    subscribedUsers.forEach(async subscriber => {
      if (subscriber.fcmToken && !uniqueFcmTokens.has(subscriber.fcmToken)) {
        uniqueFcmTokens.add(subscriber.fcmToken);
        console.log(111);
        sendNotify("New Podcast", `A new podcast "${title}" is available. Check it out!`, "podcast", subscriber.fcmToken);
      }
    });
