...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const subscribedUsers = await User.find({ _id: { $in: podcast.subscribesList.map(user => user._id) } });
      ...
      subscribedUsers.forEach(async subscriber => {
        if (subscriber.fcmToken && !uniqueFcmTokens.has(subscriber.fcmToken)) {
          ...
          console.log(111);
          sendNotify("New Episode", `A new episode "${title}" of "${podcast.title}" is available. Check it out!`, "episode", subscriber.fcmToken);
        }
      });
